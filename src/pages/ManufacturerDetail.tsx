      ...
      <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }}>
            ...
            <div>
              <div className="flex items-center gap-3 mb-2 flex-wrap">
                ...
                {manufacturer.founded && (
                  <span className="text-sm text-gray-400 font-medium">Est. {manufacturer.founded}</span>
                )}
              ...
              <h1 className="text-4xl font-black text-gray-900 mb-1">{manufacturer.shortName || manufacturer.name}</h1>
              {manufacturer.shortName && manufacturer.shortName !== manufacturer.name && (
                <p className="text-gray-400 text-sm mb-3">{manufacturer.name}</p>
              )}
              <p className="text-gray-600 leading-relaxed max-w-2xl">{manufacturer.description}</p>
            </div>
            <div className="flex flex-col gap-3 shrink-0">
              <div className="text-center bg-indigo-50 rounded-2xl px-6 py-4">
                <div className="text-3xl font-black text-indigo-600">{discs.length}</div>
                <div className="text-xs text-indigo-400 font-semibold mt-0.5">discs in catalog</div>
              </div>
              {manufacturer.website && (
                <a
                  ...
                  className="inline-flex items-center justify-center gap-2 text-sm font-bold text-gray-600 bg-gray-50 border border-gray-200 rounded-xl px-4 py-2.5 hover:border-indigo-300 hover:text-indigo-600 transition-all"
                >
        ...
        {discs.length === 0 ? (
          <div className="text-center py-16 bg-white rounded-3xl border-2 border-dashed border-gray-200">
            ...
            <p className="text-gray-500">No discs found for this manufacturer in the current catalog.</p>
          </div>
        ) : (
          categoryOrder
              ...
              return (
                <div key={cat} className="mb-10">
                  <div className="flex items-center gap-2 mb-4">
                    ...
                    <span className="text-xs text-gray-400">{byCategory[cat].length} discs</span>
                  </div>
